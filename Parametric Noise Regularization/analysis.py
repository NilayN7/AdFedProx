...
def process_df_waterbird9(train_df, val_df, params):
    process_df(train_df, val_df, params)
    loss_metrics = []
    acc_metrics = []
...
def print_accs(
    ...
    print("this is the value of early_stopping_epoch: ", early_stopping_epoch)
...
def process_df(train_df, val_df, n_groups):
    loss_metrics = []
    acc_metrics = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("--results_dir", type=str, default="logs_elementwise/")
    ...
    train_paths = [i for i in glob.glob(args.results_dir + "*_train.csv")]
    val_paths = [i for i in glob.glob(args.results_dir + "*_val.csv")]
    print("this is the value of train_paths: ", train_paths)
    print("this is the value of val_paths: ", val_paths)
    base_path = "/home/nilay/Noise_injected_federated_learning_fedP_p/"
    
    for train_path in train_paths:
        print(f"these are the results for results in {train_path}")
        train_df = pd.read_csv(os.path.join(base_path, train_path))
        val_path = [val_path for val_path in val_paths if train_path.split("_train.csv")[0] in val_path]
        val_df = pd.read_csv(os.path.join(base_path, val_path[0]))
    ...
    # test_df = pd.read_csv(test_path)
        ...
        process_df(train_df, val_df, n_groups=group_count)
        ...
        sub_exp_name = f"{train_path}"
        print(f"Downstream Accuracies for {sub_exp_name} with {group_count} groups.")
        with open(args.results_dir + "/val_accuracies.txt", "a") as text_file:
